...
<xsl:stylesheet 
  ...
  <xsl:param name="urlPrefixTargetWfs">https://portail.sig.rennesmetropole.fr/geoserver/ows?service=wfs&amp;request=GetCapabilities</xsl:param>
  <xsl:param name="urlTypeSearch">/wfs?</xsl:param>
        ...
        <xsl:when test="name()='gmd:URL'">
          <!-- le noeud gmd:URL avec le type préfédini (wms ou wfs) à été trouvé -->
				<xsl:if test="contains(text(), $urlPrefixSearch) and contains(text(), $urlTypeSearch) and not (contains(text(), $urlPrefixAfterTransf))">
					<xsl:copy-of  select="$urlPrefixTargetWfs"/>
				</xsl:if>
